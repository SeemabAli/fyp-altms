...
export default function DeletePreferenceModal({
  open,
  setOpen,
  selected,
  refresh,
  ...
  const handleDelete = async () => {
    if (!selected?._id) {
      toast.error("No preference selected");
      return;
    }

    setDeleting(true);
    try {
      const res = await fetch(`/api/coordinators/preferences/${selected._id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!res.ok)
        throw new Error(
          data?.error || data?.message || "Failed to delete preference"
        );
      ...
      toast.error(e.message || "Error deleting preference");
    } finally {
      setDeleting(false);
    }
  };
